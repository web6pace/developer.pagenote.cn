...
import { makeResize } from 'utils/resize'
...
export default function PageHead({ children, thumbs = [], initHeight }: Props) {
  ...
  useEffect(function () {
    ...
    return makeResize(ref.current, function (offsetX, offsetY) {
      changeHeight(offsetY)
      console.log(offsetY)
    })
  }, [])

  console.log(thumbs, 'thuembs')
        ...
        {thumbs.map((item, index) => (
          <SwiperSlide key={item.img}>
            <div className="overflow-hidden h-full absolute w-full left-0 top-0">
              <ImgFallback className="w-full h-fit" src={item.img} alt="" />
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
